...
const SYS = `너는 의료소모품 유통사의 정산 보조자다. 거래처가 보낸 [출고확인서/거래명세서](텍스트 또는 이미지)를 읽고,
[발주품목] 목록의 각 줄에 실제 출고된 수량·단가를 매칭한다. JSON 객체만 출력한다.

출력 형식: {"vendor":"거래처명(있으면, 없으면 \\"\\")","shipping_fee":배송비숫자,"items":[{"line_index":번호,"doc_name":"명세서품목명","qty":출고수량,"unit_price":단가}]}
...
export async function POST(request: Request) {
  ...
  const { data: rows } = await supabase
    .from("demand_lines")
    .select("id, raw_name, sort_order, product:products(name, spec)")
    .eq("job_id", jobId);
